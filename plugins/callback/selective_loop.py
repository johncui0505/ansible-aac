...
class CallbackModule(CallbackBase):
    """selective_loop.py callback plugin."""

    CALLBACK_VERSION = 2.0
    CALLBACK_TYPE = "stdout"
    CALLBACK_NAME = "selective_loop"
    ...
    def _print_task(self, task_name=None):
        ...
        if not self.printed_last_task:
            ...
            if self.last_skipped:
                print()
            msg = colorize(
                "# {0} {1}".format(task_name, "*" * (line_length - len(task_name))),
                "bold",
            )
            print(msg)
    ...
    def _indent_text(self, text, indent_level):
        ...
        result_lines = []
        for line in lines:
            result_lines.append("{0}{1}".format(" " * indent_level, line))
        return "\n".join(result_lines)
    ...
    def _print_host_or_item(
        ...
        line = "{0}  * {1}{2}- {3}".format(
            " " * indent_level, name, spaces, change_string
        )
    ...
    def _print_task_result(self, result, error=False, **kwargs):
        ...
        if "print_action" in result._task.tags or error or self._display.verbosity > 1:
            self._print_task()
            self.last_skipped = False
            msg = to_text(result._result.get("msg", "")) or to_text(
