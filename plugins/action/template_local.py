...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            trim_blocks = boolean(
                self._task.args.get("trim_blocks", True), strict=False
            )
            lstrip_blocks = boolean(
                self._task.args.get("lstrip_blocks", False), strict=False
            )
        ...
        if newline_sequence in wrong_sequences:
            newline_sequence = allowed_sequences[
        ...
        try:
            ...
            try:
                ...
                if variable_start_string is not None:
                    self._templar.environment.variable_start_string = (
                        variable_start_string
                    )
                ...
                temp_vars.update(
                    generate_ansible_template_vars(path=source, dest_path=dest)
                )
            ...
            try:
                ...
                if os.path.isfile(dest) and filecmp.cmp(result_file, dest):
                    result["changed"] = False
                else:
                    result["changed"] = True
                shutil.copyfile(result_file, dest)
            finally:
